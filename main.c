...
int lsh_pwd(char **args)
{
  ...
  if (args[1] != NULL)
  {
    printf("No extra arguments rather than pwd acceptable\n");
    return 1;
  }
  char cwd[1024];
...
int lsh_env(char **args)
{
  if (args[1] != NULL)
  {
    printf("JUST the env command is acceptable no more!\n");
    return 1;
  }
  int i = 0;
...
int lsh_export(char **args)
{
  ...
  if (s[i] == '\0')
  {
    printf("Invalid format: missing '='. Use: export NAME=VALUE\n");
    return 1;
  }

  if (i == 0)
  {
    printf("Invalid format: NAME cannot be empty. Use: export NAME=VALUE\n");
    return 1;
  }
